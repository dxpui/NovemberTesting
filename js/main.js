...
//     const limit = parseInt($input.attr("data-maxlength"), 10) || 250; // default fallback
...
$(document).ready(function () {
    const $inputs = $(".search-box-input");

    $inputs.each(function () {
        const $input = $(this);
        const $error = $input.closest(".search-box").siblings(".search-error-message");
        const limit = parseInt($input.attr("data-maxlength"), 10) || 250;
...
$(document).ready(function () {
    // Toggle orange color on click
    $(document).on("click", ".filters-content", function () {
...
$(function () {
    ...
    $(".accordion-head").each(function (index) {
        ...
        const link = $('<a/>', {
            ...
            "aria-expanded": "false",
            tabindex: "0",
            "data-bs-toggle": tooltipDesc ? "tooltip" : "",
    ...
    // 4️⃣ Toggle accordion (click or keyboard)
    function toggleAccordion($link) {
        const target = $link.data("target");
        const $collapse = $(target);
        const bsCollapse = bootstrap.Collapse.getOrCreateInstance($collapse[0], { toggle: false });
    ...
    $(document).on("click", "#link-tags .btn-tag", function () {
        toggleAccordion($(this));
    });

    $(document).on("keydown", "#link-tags .btn-tag", function (e) {
    ...
    $(document).on("hidden.bs.collapse", "#myAccordion .accordion-collapse", function () {
        const targetId = `#${this.id}`;
        const $link = $(`#link-tags .btn-tag[data-target='${targetId}']`);
    ...
    const collapseText = $toggleAll.data("collapse-text") || "Collapse All accordion";
    ...
    function toggleAllAccordions() {
        ...
        $("#myAccordion .accordion-collapse").each(function () {
            const bsCollapse = bootstrap.Collapse.getOrCreateInstance(this, { toggle: false });
            if (allOpen) bsCollapse.show();
            else bsCollapse.hide();
        });

        $toggleAll.find(".toggle-text").text(allOpen ? collapseText : expandText);
        $toggleAll.find(".toggle-icon").toggleClass("fa-angle-down fa-angle-up");
        $toggleAll.attr("aria-expanded", allOpen.toString());
    }

    $toggleAll.on("click", toggleAllAccordions);
    $toggleAll.on("keydown", function (e) {
        if (e.key === "Enter" || e.key === " ") {
            e.preventDefault();
            toggleAllAccordions();
        }
